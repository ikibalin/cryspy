...
class cwidget(widg_min.cwidget_min):
    ...
    def init_widget(self, model):
        ...
        self.cb_exp = QtWidgets.QComboBox()
        lname = [exp.name for exp in model.exp]
        lname.append("new experiment")
        self.cb_exp.addItems(lname)

        for iexp, exp in enumerate(model.exp):
            exp.add_obs_labs_pars(self, "cb_exp", iexp, "name", None)


        lay_central.addWidget(self.cb_ph)
        lay_central.addWidget(self.cb_exp)
