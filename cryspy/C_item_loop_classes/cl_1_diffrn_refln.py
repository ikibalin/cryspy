...
class DiffrnReflnL(LoopN):
    ...
    def report(self):
        return (
            self.report_agreement_factor_exp()
            + "\n"
    ...
    def report_agreement_factor_exp(self):
        ...
        l_hkl = [
            ...
            for _1, _2, _3 in zip(self.index_h, self.index_k, self.index_l)
        ]
        flag_fr = self.is_attribute("fr") and self.is_attribute("fr_sigma")
        flag_intensity = self.is_attribute("intensity") and self.is_attribute(
            "intensity_sigma"
        )
        if flag_fr:
            l_val = self.fr
        ...
        for _hkl, _val_1, _sval_1 in zip(l_hkl, l_val, l_sval):
            if flag_fr:
                _diff = abs(_val_1 - 1.0) / float(_sval_1)
                if _diff >= 3.0:
                    n_3s += 1
                elif _diff >= 2.0:
                    n_2s += 1
                    ...
                    n_0s += 1
                if _val_1 <= 1.0:
                    if math.isclose(_val_1, 0.0):
                        l_diff.append(1.0 / (_val_1 + _sval_1) - 1.0)
                    else:
                        l_diff.append(1.0 / _val_1 - 1.0)
                else:
                    l_diff.append(_val_1 - 1.0)

            _mhkl = (-1 * _hkl[0], -1 * _hkl[1], -1 * _hkl[2])
            if _mhkl in l_hkl:
                ind_mhkl = l_hkl.index(_mhkl)
                _val_2, _sval_2 = l_val[ind_mhkl], l_sval[ind_mhkl]
                _sval = 1.0 / (_sval_1 ** (-2) + _sval_2 ** (-2)) ** 0.5
                _val_average = (
                    _val_1 * _sval_1 ** (-2) + _val_2 * _sval_2 ** (-2)
                ) * _sval**2
                delta_val = abs(_val_1 - _val_average)
                chi_sq_exp = (delta_val / _sval_1) ** 2
                l_chi_sq_exp.append(chi_sq_exp)
                if math.isclose(_val_1 - 1.0, 0):
                    ag_exp = abs(
                        (_val_1 - _val_average) / (_val_1 - 1.0 + _sval_1)
                    )
                else:
                    ag_exp = abs(
                        (_val_1 - _val_average) / (_val_1 - 1.0 + _sval_1)
        ...
        ls_out.append(
            f"  |    range of h is |{min([_[0] for _ in l_hkl]):3},{max([_[0] for _ in l_hkl]):3} |"
        )
        ls_out.append(
            f"  |             k is |{min([_[1] for _ in l_hkl]):3},{max([_[1] for _ in l_hkl]):3} |"
        )
        ls_out.append(
            f"  |             l is |{min([_[2] for _ in l_hkl]):3},{max([_[2] for _ in l_hkl]):3} |"
        )
        if flag_fr:
            ls_out.append(f" |max(FR_exp - 1) is |{max(l_diff):5.3f} |")
            ls_out.append("\n N+1 > abs(FR_exp - 1)/FR_sigma > N: ")
            ls_out.append(
                f" |abs(FR_exp - 1)/FR_sigma < 1: |{n_0s:4}| {100*float(n_0s)/float(n):5.1f}% | "
            )
            ls_out.append(
                f" |                    N = 1: |{n_1s:4}| {100*float(n_1s)/float(n):5.1f}%  |"
            )
            ls_out.append(
                f" |                    N = 2: |{n_2s:4}| {100 * float(n_2s) / float(n):5.1f}% |"
            )
            ls_out.append(
                f" |abs(FR_exp - 1)/FR_sigma > 3: |{n_3s:4}| {100 * float(n_3s) / float(n):5.1f}% |"
            )
        ...
        if n_friedel != 0:
            ls_out.append(
                f"|  (abs(FR_exp-FR_av.)/FR_sigma)^2  is| {sum(l_chi_sq_exp) / n_friedel:.2f}|"
            )
            ls_out.append(
                f"|   abs(FR_exp-FR_av.)/abs(FR_exp-1) per reflection is |{(100*sum(l_ag_exp)/n_friedel):.2f}% |"
            )
    ...
    def calc_chi_sq_points(self) -> (float, int):
        ...
        if (
            ...
            chi_sq = numpy.sum(
                numpy.square((np_fr - np_fr_calc) / np_fr_sigma)
            )
            points = np_fr.size
    ...
    def report_chi_sq_exp(self, cell=None) -> str:
        ...
        flag_fr = (
            self.is_attribute("fr")
            and self.is_attribute("fr_sigma")
            and self.is_attribute("fr_calc")
        )
        flag_intensity = (
            self.is_attribute("intensity")
            and self.is_attribute("intensity_sigma")
            and self.is_attribute("intensity_calc")
        )
        if flag_fr:
            l_val, l_sval, l_val_calc = self.fr, self.fr_sigma, self.fr_calc
        elif flag_intensity:
            l_val, l_sval, l_val_calc = (
                self.intensity,
        ...
        for _hkl, _val, _sval, _val_calc in zip(
            l_hkl, l_val, l_sval, l_val_calc
        ):
            _diff = abs(float(_val - _val_calc) / float(_sval))
            l_chi_sq.append(_diff**2)
            if math.isclose(_val, 0.0) and not math.isclose(_sval, 0.0):
                l_af_f.append(abs(float(_val - _val_calc) / float(_sval)))
            else:
                l_af_f.append(abs(float(_val - _val_calc) / float(_val)))
            if not (math.isclose(_val, 1.0)) and flag_fr:
                l_af_r.append(abs(float(_val - _val_calc) / float(_val - 1.0)))
            if _diff <= 1.0:
                n_1s += 1
            elif _diff <= 2.0:
                n_2s += 1
        ...
        if flag_fr:
            ls_out.append(
                f"|  (abs(FR_exp-FR_mod)/FR_sigma)^2 per reflection is |{sum(l_chi_sq)/float(n):.2f}|"
            )
            ls_out.append(
                f"|   abs(FR_exp-FR_mod)/FR_exp      per reflection is |{100*sum(l_af_f) / float(n):.2f}%|"
            )
            ls_out.append(
                f"|   abs(FR_exp-FR_mod)/abs(FR_exp-1)    per reflection is |{100*sum(l_af_r) / float(n):.2f}%|"
            )
            ls_out.append(
                "           (reflections with FR_exp = 1 are excluded)"
            )
        elif flag_intensity:
            ls_out.append(
                f"|  (abs(Int_exp-Int_mod)/Int_sigma)^2 per reflection is |{sum(l_chi_sq)/float(n):.2f}|"
            )
            ls_out.append(
                f"|   abs(Int_exp-Int_mod)/Int_exp      per reflection is |{100*sum(l_af_f) / float(n):.2f}%|"
            )
        n_worsest = len(l_worsest)
        ls_out.append("\n## Reflections in range  ")
        if flag_fr:
            ls_out.append(
                " (N-1)*FR_sigma < abs(FR_exp - FR_mod) < N*FR_sigma: "
            )
        elif flag_intensity:
            ls_out.append(
                " (N-1)*Int_sigma < abs(Int_exp - Int_mod) < N*Int_sigma: "
            )
        ls_out.append(
            f"|      N = 1:| {n_1s:}/{n:} ={100*float(n_1s)/float(n):5.1f}% ({2*34.1:4.1f}%, three sigma rule) |"
        )
        ls_out.append(
            f"|      N = 2:| {n_2s:}/{n:} ={100 * float(n_2s) / float(n):5.1f}% ({2*(13.6):4.1f}%, three sigma rule)|"
        )
        ls_out.append(
            f"|      N = 3:| {n_3s:}/{n:} ={100 * float(n_3s) / float(n):5.1f}% ({2*(2.1):4.1f}%, three sigma rule)|"
        )
        ls_out.append(
            f"|      N > 3:| {n_worsest:}/{n:} ={100 * float(n_worsest) / float(n):5.1f}% ({2*(0.1):4.1f}%, three sigma rule)|"
        )
        ...
        if len(l_worsest) > 1:
            ...
            if flag_fr:
                ls_out.append(
                    "|  h | k | l  |     FR |FR_sigma | FR_calc | diff|"
                )
            elif flag_intensity:
                ls_out.append(
                    "|  h | k | l  |     Int |Int_sigma | Int_calc | diff|"
                )
            for _hkl, _fr, _fr_sigma, _fr_calc, _diff in l_worsest[:n_worsest]:
                ls_out.append(
                    f"|{_hkl[0]:3}|{_hkl[1]:3}|{_hkl[2]:3}|{_fr:9.5f}|{_fr_sigma:9.5f}|{_fr_calc:9.5f}|{_diff:6.1f}|"
                )
        if cell is not None:
            np_h = numpy.array(self.index_h, dtype=int)
            np_k = numpy.array(self.index_k, dtype=int)
            np_l = numpy.array(self.index_l, dtype=int)
            np_sthovl = cell.calc_sthovl(
                index_h=np_h, index_k=np_k, index_l=np_l
            )
            np_val, np_sval = numpy.array(l_val, dtype=float), numpy.array(
                l_sval, dtype=float
            )
            ...
            for _1, _2, _3 in zip(
                np_sthovl_bins[:-1], np_sthovl_bins[1:], np_val
            ):
                np_flag = numpy.logical_and(np_sthovl >= _1, np_sthovl < _2)
                res = np_diff[np_flag]
                if res.size == 0:
                    ls_out.append(
                        f"|{_1:8.3f} |{_2:8.3f}|       None    |      0|"
                    )
                else:
                    ls_out.append(
                        f"|{_1:8.3f} |{_2:8.3f}|    {np_diff[np_flag].mean():7.3f} |{_3:10}|"
                    )
    ...
    def plot_fr_vs_fr_calc(self):
        ...
        if numpy.all(np_excl):
            ...
            chi_sq_per_n = (
                numpy.square((np_fr - np_fr_calc) / np_fr_sigma).sum()
                / np_fr.shape[0]
            )
            ax.set_title(
                r"Flip Ratio: $I_{plus} / I_{minus}$, $\chi^2/n=$"
                + f"{chi_sq_per_n:.2f}."
            )
    ...
    def plot_hist_asymmetry_calc(self):
        ...
        ls_label = [
            f"{h} {k} {l}"
            for h, k, l in zip(self.index_h, self.index_k, self.index_l)
        ]
        fig, ax = plt.subplots()
        np_excl = numpy.array(self.excluded, dtype=bool)
        flag_in = numpy.logical_not(np_excl)
        ...
        # plot
        x = 0.5 + numpy.arange(asymmetry_calc.size)
        ax.bar(
            x,
            asymmetry,
            width=1,
    ...
    def plot_intensity_vs_intensity_calc(self):
        ...
        if numpy.all(np_excl):
            ...
            np_intensity_sigma = numpy.array(
                self.intensity_sigma, dtype=float
            )[flag_in]

            chi_sq_per_n = (
                numpy.square(
                    (np_intensity - np_intensity_calc) / np_intensity_sigma
                ).sum()
                / np_intensity.shape[0]
            )
            agreement_factor = (
                numpy.abs(np_intensity - np_intensity_calc).sum()
                / numpy.abs(np_intensity).sum()
                * 100.0
            )
            ax.set_title(
                r"Intensity, $\chi^2/n=$"
            ...
            intensity_min = min(
                [min(np_intensity_1), min(self.intensity_calc)]
            )
            intensity_max = max(
                [max(np_intensity_2), max(self.intensity_calc)]
            )
            ax.plot(
    ...
    def exclude_by_fr_max(self, fr_max: float = 3.0):
