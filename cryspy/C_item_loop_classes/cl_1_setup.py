...
class Setup(ItemN):
    ...
    def __init__(self, **kwargs) -> NoReturn:
        ...
        D_MIN = {"wavelength": 0.0, "ratio_lambdaover2": 0.0}

        # defined for ani integer and float parameters
        D_MAX = {"ratio_lambdaover2": 1.0}
    ...
    def get_dictionary(self):
        ...
        if self.is_attribute("wavelength"):
            ...
            res["flags_wavelength"] = numpy.array(
                [self.wavelength_refinement], dtype=bool
            )
        if self.is_attribute("offset_ttheta"):
            if self.offset_ttheta is not None:
                res["offset_ttheta"] = numpy.array(
                    [self.offset_ttheta * numpy.pi / 180.0], dtype=float
                )
                res["flags_offset_ttheta"] = numpy.array(
                    [self.offset_ttheta_refinement], dtype=bool
                )
        if self.is_attribute("offset_sycos"):
            if self.offset_sycos is not None:
                res["offset_sycos"] = numpy.array(
                    [self.offset_sycos], dtype=float
                )
                res["flags_offset_sycos"] = numpy.array(
                    [self.offset_sycos_refinement], dtype=bool
                )
        if self.is_attribute("offset_sysin"):
            if self.offset_sysin is not None:
                res["offset_sysin"] = numpy.array(
                    [self.offset_sysin], dtype=float
                )
                res["flags_offset_sysin"] = numpy.array(
                    [self.offset_sysin_refinement], dtype=bool
                )
        if self.is_attribute("offset_gamma"):
            if self.offset_gamma is not None:
                res["offset_gamma"] = numpy.array(
                    [self.offset_gamma * numpy.pi / 180.0], dtype=float
                )
                res["flags_offset_gamma"] = numpy.array(
                    [self.offset_gamma_refinement], dtype=bool
                )
        if self.is_attribute("offset_nu"):
            if self.offset_nu is not None:
                res["offset_nu"] = numpy.array(
                    [self.offset_nu * numpy.pi / 180.0], dtype=float
                )
                res["flags_offset_nu"] = numpy.array(
                    [self.offset_nu_refinement], dtype=bool
                )
        ...
        if self.is_attribute("ratio_lambdaover2"):
            res["c_lambda2"] = numpy.array(
                [self.ratio_lambdaover2], dtype=float
            )
            res["flags_c_lambda2"] = numpy.array(
                [self.ratio_lambdaover2_refinement], dtype=bool
            )
