...
class Pd(DataN):
    ...
    def get_dictionary(self):
        ...
        if pd_meas is not None:
            ...
            flag_in = numpy.logical_and(
                ttheta_deg >= ttheta_min_deg, ttheta_deg <= ttheta_max_deg
            )

            ddict["ttheta"] = ttheta_deg[flag_in] * numpy.pi / 180.0
            if pd_meas.is_attribute("intensity_plus"):
                int_plus = numpy.array(pd_meas.intensity_plus, dtype=float)[
                    flag_in
                ]
                s_int_plus = numpy.array(
                    pd_meas.intensity_plus_sigma, dtype=float
                )[flag_in]
                int_minus = numpy.array(pd_meas.intensity_minus, dtype=float)[
                    flag_in
                ]
                s_int_minus = numpy.array(
                    pd_meas.intensity_minus_sigma, dtype=float
                )[flag_in]
                ddict["signal_exp_plus"] = numpy.stack(
                    [int_plus, s_int_plus], axis=0
                )
                ddict["signal_exp_minus"] = numpy.stack(
                    [int_minus, s_int_minus], axis=0
                )
            else:
                int_sum = numpy.array(pd_meas.intensity, dtype=float)[flag_in]
                s_int_sum = numpy.array(pd_meas.intensity_sigma, dtype=float)[
                    flag_in
                ]
    ...
    def take_parameters_from_dictionary(
        self, ddict_diffrn, l_parameter_name: list = None, l_sigma: list = None
    ):
        ...
        if "beam_polarization" in keys:
            self.diffrn_radiation.polarization = ddict_diffrn[
                "beam_polarization"
            ]

        if "flipper_efficiency" in keys:
            self.diffrn_radiation.efficiency = ddict_diffrn[
                "flipper_efficiency"
            ]
        ...
        if "offset_ttheta" in keys:
            self.setup.offset_ttheta = (
                float(ddict_diffrn["offset_ttheta"].squeeze())
                * 180.0
                / numpy.pi
            )

        if "offset_sycos" in keys:
            self.setup.offset_sycos = float(
                ddict_diffrn["offset_sycos"].squeeze()
            )

        if "offset_sysin" in keys:
            self.setup.offset_sysin = float(
                ddict_diffrn["offset_sysin"].squeeze()
            )
        ...
        if len(parameter_label) > 0:
            for name, sigma in zip(l_parameter_name, l_sigma):
                if name[0] == "background_intensity":
                    self.pd_background.items[name[1][0]].intensity_sigma = (
                        float(sigma)
                    )
                ...
                if name[0] == "offset_ttheta":
                    self.setup.offset_ttheta_sigma = (
                        float(sigma) * 180.0 / numpy.pi
                    )
                if name[0] == "offset_sycos":
                    self.setup.offset_sycos_sigma = float(sigma)
                if name[0] == "offset_sysin":
                    self.setup.offset_sysin_sigma = float(sigma)
        ...
        if ("signal_plus" in keys) and ("signal_minus" in keys):
            pd_proc = PdProcL()
            pd_proc.numpy_ttheta = numpy.round(
                ddict_diffrn["ttheta"] * 180.0 / numpy.pi, decimals=5
            )
            pd_proc.numpy_ttheta_corrected = numpy.round(
                ddict_diffrn["ttheta_corrected"] * 180.0 / numpy.pi, decimals=5
            )
            pd_proc.numpy_intensity_plus_net = numpy.round(
                ddict_diffrn["signal_plus"], decimals=5
            )
            pd_proc.numpy_intensity_minus_net = numpy.round(
                ddict_diffrn["signal_minus"], decimals=5
            )
            if "signal_exp_plus" in keys:
                pd_proc.numpy_intensity_plus = numpy.round(
                    ddict_diffrn["signal_exp_plus"][0], decimals=5
                )
                pd_proc.numpy_intensity_plus_sigma = numpy.round(
                    ddict_diffrn["signal_exp_plus"][1], decimals=5
                )
                pd_proc.numpy_intensity_minus = numpy.round(
                    ddict_diffrn["signal_exp_minus"][0], decimals=5
                )
                pd_proc.numpy_intensity_minus_sigma = numpy.round(
                    ddict_diffrn["signal_exp_minus"][1], decimals=5
                )
            else:
                pd_proc.numpy_intensity = numpy.round(
                    ddict_diffrn["signal_exp"][0], decimals=5
                )
                pd_proc.numpy_intensity_sigma = numpy.round(
                    ddict_diffrn["signal_exp"][1], decimals=5
                )
            pd_proc.numpy_intensity_bkg_calc = numpy.round(
            ...
            if s_label in keys:
                ...
                if "index_hkl" in dict_crystal_keys:
                    ...
                    if "iint_plus_with_factors" in dict_crystal_keys:
                        pd_peak.numpy_intensity_plus = dict_crystal[
                            "iint_plus_with_factors"
                        ]
                    if "iint_minus_with_factors" in dict_crystal_keys:
                        pd_peak.numpy_intensity_minus = dict_crystal[
                            "iint_minus_with_factors"
                        ]
                    if "multiplicity_hkl" in dict_crystal_keys:
                        pd_peak.numpy_index_mult = dict_crystal[
                            "multiplicity_hkl"
                        ]
                    if "ttheta_hkl" in dict_crystal_keys:
                        pd_peak.numpy_ttheta = numpy.round(
                            dict_crystal["ttheta_hkl"] * 180.0 / numpy.pi,
                            decimals=3,
                        )
                    ...
                    if ("iint_plus" in dict_crystal_keys) and (
                        "iint_minus" in dict_crystal_keys
                    ):
                        f_squared_calc = (
                            dict_crystal["iint_plus"]
                            + dict_crystal["iint_minus"]
                        )
                        f_polarized_squared_calc = (
                            dict_crystal["iint_plus"]
                            - dict_crystal["iint_minus"]
                        )
                        flag_f_squared = True
                    ...
                    if (
                        ("f_nucl" in dict_crystal_keys)
                        or ("f_charge" in dict_crystal_keys)
                    ) or flag_f_squared:
                        ...
                        if flag_f_squared:
                            ...
                            refln.numpy_f_polarized_squared_calc = (
                                f_polarized_squared_calc
                            )
    ...
    def define_background_points(self, step_ttheta: float = 1.0):
    ...
    def estimate_background(self):
        pd_background = self.pd_background
        intensity_refinement = numpy.copy(
            numpy.array(pd_background.intensity_refinement, dtype=bool)
        )
        pd_background.numpy_intensity_refinement = numpy.ones_like(
            intensity_refinement, dtype=bool
        )
        pd_background.numpy_to_items()
        setup = self.setup
        pd_proc = self.pd_proc
        res = pd_proc.estimate_background(
            pd_background, offset_ttheta=setup.offset_ttheta
        )
