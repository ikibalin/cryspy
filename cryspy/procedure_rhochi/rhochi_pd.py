...
def calc_background(
    ...
    flag_background_intensity: bool = False,
):
    ...
    if x_p.max() <= x_max:
        x_max = x_max + 1.0
        y_last = (y_p[-1] - y_p[-2]) * (x_max - x_p[-2]) / (
            x_p[-1] - x_p[-2]
        ) + y_p[-2]
    ...
    x_left = x_p[:-1]
    x_right = x_p[1:]
    flags = numpy.logical_and(
        ttheta[:, na] >= x_left[na, :], ttheta[:, na] < x_right[na, :]
    )
    p0 = numpy.argwhere(flags)[:, 1]
    p1 = p0 + 1
    intensity = (y_p[p1] - y_p[p0]) * (ttheta - x_p[p0]) / (
        x_p[p1] - x_p[p0]
    ) + y_p[p0]
    ...
    if flag_background_intensity:
        ...
        dder_bkgr = numpy.zeros(
            (ttheta.size, background_ttheta.size), dtype=float
        )
...
def calc_chi_sq_for_pd_by_dictionary(
    ...
    offset_sycos = dict_pd.get("offset_sycos", 0.0)
    offset_sysin = dict_pd.get("offset_sysin", 0.0)
    ttheta_zs = ttheta - (
        offset_ttheta
        + numpy.radians(offset_sycos) * numpy.cos(0.5 * ttheta)
        + numpy.radians(offset_sysin) * numpy.sin(0.5 * ttheta)
    )
    ...
    if "beam_polarization" in dict_pd_keys:
        ...
        beam_polarization, flipper_efficiency, magnetic_field = 0.0, 0.0, 0.0
        flags_beam_polarization, flags_flipper_efficiency = False, False

    sthovl_min = numpy.sin(
        0.5 * ttheta_zs.min() - numpy.pi / 90.0
    ) / numpy.max(wavelength)
    if sthovl_min <= 0:
        sthovl_min = 0.0001
    sthovl_max = numpy.sin(
        0.5 * ttheta_zs.max() + numpy.pi / 90.0
    ) / numpy.min(wavelength)
    ...
    if (
        ...
        and ("signal_background" in dict_in_out)
        ...
        signal_background, dder_s_bkgr = calc_background(
            ...
            background_ttheta,
            background_intensity,
            flag_background_intensity=(
                flag_background_intensity and flag_calc_analytical_derivatives
            ),
        )
    ...
    pd_phase_resolution_parameters = dict_pd[
        "phase_resolution_parameters"
    ]  # U_phase, V_phase, W_phase, X_phase, Y_phase
    ...
    flags_pd_phase_resolution_parameters = dict_pd[
        "flags_phase_resolution_parameters"
    ]  # U_phase, V_phase, W_phase, X_phase, Y_phase
    ...
    if "asymmetry_parameters" in dict_pd_keys:
        ...
        p_1, p_2, p_3, p_4 = (
            asymmetry_parameters[0],
            asymmetry_parameters[1],
            asymmetry_parameters[2],
            asymmetry_parameters[3],
        )
    ...
    lorentz_factor, dder_lf = calc_lorentz_factor(
        ttheta_zs, k=k, cthm=cthm, flag_ttheta=flag_offset
    )
    ...
    for (
        ...
        pd_phase_name,
        pd_phase_scale,
        pd_phase_resolution_parameters.transpose(),
        pd_phase_ig,
        ...
        if (
            ...
            and not (flag_unit_cell_parameters or flag_offset)
        ):
        ...
        if radiation[0].startswith("neutrons"):
            ...
            flag_para = False
            if ("atom_para_index" in dict_crystal_keys) and (
                "atom_para_susceptibility" in dict_crystal_keys
            ):
            ...
            if "atom_ordered_index" in dict_crystal_keys:
                f_m_perp_o_ccs, dder_f_m_perp_o_ccs = (
                    calc_f_m_perp_ordered_by_dictionary(
                        dict_crystal,
                        dict_in_out_phase,
                        flag_use_precalculated_data=flag_use_precalculated_data,
            ...
            if flag_para and not (flag_ordered):
                flag_iint_plus_minus = (
                    flag_f_nucl
                    or flag_tensor_sigma
                    or flags_beam_polarization
                    or flags_flipper_efficiency
                )
                if (
                    ("iint_plus" in dict_in_out_phase_keys)
                    and ("iint_minu" in dict_in_out_phase_keys)
                    and flag_use_precalculated_data
                    and not (flag_iint_plus_minus)
                ):
                    iint_plus, iint_minus = (
                        dict_in_out_phase["iint_plus"],
                        dict_in_out_phase["iint_minus"],
                    )
                else:
        ...
        if flag_phase_texture:
            ...
            if (
                ...
                dict_in_out_phase["preferred_orientation"] = (
                    preferred_orientation
                )
        ...
        lf = calc_lorentz_factor(ttheta_hkl, k=k, cthm=cthm, flag_ttheta=None)[
            0
        ]
        dict_in_out_phase["iint_plus_with_factors"] = (
            0.5 * p_scale * lf * iint_m_plus
        )
        dict_in_out_phase["iint_minus_with_factors"] = (
            0.5 * p_scale * lf * iint_m_minus
        )
        if flag_texture:
            ...
            signal_plus = (
                ...
                * lorentz_factor
                * (
                    profile_pv * (iint_m_plus * preferred_orientation)[na, :]
                ).sum(axis=1)
            )  # sum over hkl
            signal_minus = (
                0.5
                ...
                * lorentz_factor
                * (
                    profile_pv * (iint_m_minus * preferred_orientation)[na, :]
                ).sum(axis=1)
            )
            dict_in_out_phase[
    ...
    if ("signal_exp_plus" in dict_pd_keys) and (
        "signal_exp_minus" in dict_pd_keys
    ):
    ...
    if flag_chi_sq_sum:
        ...
        total_signal_sum = (
            total_signal_plus + total_signal_minus + signal_background
        )
        chi_sq_sum = (
            (
                numpy.square((signal_exp - total_signal_sum) / signal_sigma)
                * in_points
            )
        ).sum(axis=0)
        chi_sq += chi_sq_sum
    ...
    if flag_chi_sq_difference:
        ...
        chi_sq_diff = (
            numpy.square(
                (signal_exp_diff - total_signal_diff) / signal_sigma_diff
            )
        ).sum(axis=0)
        chi_sq += chi_sq_diff
    ...
    for way, flags in flags_pd.items():
        ...
        parameter_name = [
            (pd_type_name,) + way + (tuple(ind_1d[ind, :]),)
