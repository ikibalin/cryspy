...
from cryspy.procedure_mempy.mempy_by_dictionary import (
    ...
    mempy_cycle_density_parameters,
)
...
def mempy_spin_density_reconstruction(obj: cryspy.GlobalN):
    if not (obj.is_attribute("mem_parameters")):
        mem_parameters = cryspy.MEMParameters(
            ...
            points_c=48,
            channel_col=True,
            magnetization_plus=4,
            magnetization_minus=-1,
        )
        obj.add_items(
            [
                mem_parameters,
...
def mempy_reconstruction_with_parameters(
    obj: cryspy.GlobalN,
    ...
    l_dict_mem_parameters = [
        value
        for name, value in dict_obj.items()
        if name.startswith("mem_parameters")
    ]
    l_dict_diffrn = [
        value for name, value in dict_obj.items() if name.startswith("diffrn_")
    ]
    ...
    if len(l_dict_crystal) == 0:
        ...
        raise UserWarning(
            "The first crystal will be used for MEM reconstruction procedure"
        )

    if len(l_dict_diffrn) == 0:
        raise KeyError(
            "The single crystall polarized experiments are not found"
        )

    dict_crystal = l_dict_crystal[0]
    dict_mem_parameters = l_dict_mem_parameters[0]
    dict_in_out = {}
    res = mempy_reconstruction_by_dictionary(
        dict_crystal,
        dict_mem_parameters,
        l_dict_diffrn,
        dict_in_out,
    ...
    for diffrn in l_diffrn:
        flag_pol = (
            "flip_ratio"
            in dict_in_out["dict_in_out_" + diffrn.get_name()].keys()
        )
        diffrn_refln = diffrn.diffrn_refln
        if flag_pol:
            flip_ratio = dict_in_out["dict_in_out_" + diffrn.get_name()][
                "flip_ratio"
            ]
            diffrn_refln.numpy_fr_calc = flip_ratio
        else:
            intensity = dict_in_out["dict_in_out_" + diffrn.get_name()][
                "intensity"
            ]
        ...
        refine_ls.number_reflns = numpy.logical_not(
            diffrn_refln.excluded
        ).sum()
    if (
        ("symm_elem_channel_ani" in dict_in_out_keys)
        and ("density_channel_ani" in dict_in_out_keys)
        and (
            ("susceptibility_channel_ani" in dict_in_out_keys)
            or ("moment_channel_ani" in dict_in_out_keys)
        )
        and ("atom_multiplicity_channel_ani" in dict_in_out_keys)
        and ("point_multiplicity_channel_ani" in dict_in_out_keys)
    ):

        symm_elem_channel_ani = dict_in_out["symm_elem_channel_ani"]
        density_channel_ani = dict_in_out["density_channel_ani"]

        atom_multiplicity_channel_ani = dict_in_out[
            "atom_multiplicity_channel_ani"
        ]
        point_multiplicity_channel_ani = dict_in_out[
        ...
        if "susceptibility_channel_ani" in dict_in_out_keys:
            susceptibility_channel_ani = dict_in_out[
    ...
    if (
        ("symm_elem_channel_col" in dict_in_out_keys)
        and ("magnetization_plus" in dict_in_out_keys)
        and ("magnetization_minus" in dict_in_out_keys)
        and ("density_channel_col" in dict_in_out_keys)
        and ("point_multiplicity_channel_col" in dict_in_out_keys)
    ):
        ...
        point_multiplicity_channel_col = dict_in_out[
            "point_multiplicity_channel_col"
        ]
...
def mempy_cycle_with_parameters(
    obj: cryspy.GlobalN,
    parameter_lambda: float = 1.0e-5,
    iteration_max: int = 1000,
    parameter_lambda_min: float = 1.0e-9,
    ...
    if "atom_para_susceptibility_sigma" in dict_in_out_susc.keys():
        atom_para_susceptibility_sigma = dict_in_out_susc[
            ...
            for i_attr, attr in enumerate(l_attr):
                ...
                if flags_atom_para_susceptibility[i_attr, i_item]:
                    setattr(
                        item,
                        f"{attr:}_sigma",
                        atom_para_susceptibility_sigma[i_attr, i_item],
                    )

    dict_in_out_den_keys = dict_in_out_den.keys()

    for diffrn in l_diffrn:
        flip_ratio = dict_in_out_den["dict_in_out_" + diffrn.get_name()][
            "flip_ratio"
        ]
        diffrn_refln = diffrn.diffrn_refln
        diffrn_refln.numpy_fr_calc = flip_ratio
    ...
    if (
        ("symm_elem_channel_ani" in dict_in_out_den_keys)
        and ("density_channel_ani" in dict_in_out_den_keys)
        and (
            ("susceptibility_channel_ani" in dict_in_out_den_keys)
            or ("moment_channel_ani" in dict_in_out_den_keys)
        )
        and ("atom_multiplicity_channel_ani" in dict_in_out_den_keys)
        and ("point_multiplicity_channel_ani" in dict_in_out_den_keys)
    ):
    ...
    if (
        ("symm_elem_channel_col" in dict_in_out_den_keys)
        and ("magnetization_plus" in dict_in_out_den_keys)
        and ("magnetization_minus" in dict_in_out_den_keys)
        and ("density_channel_col" in dict_in_out_den_keys)
        and ("point_multiplicity_channel_col" in dict_in_out_den_keys)
    ):
        ...
        point_multiplicity_channel_col = dict_in_out_den[
            "point_multiplicity_channel_col"
        ]
