...
def calc_f_asym_by_pr(
    ...
    res = (pr_2*(pr_1*atom_multiplicity[na, na, :]*scat_length_neutron[na, na, :]*atom_occupancy[na, na, :]*debye_waller).sum(axis=2)).sum(axis=1)/pr_2.shape[-1]
    dder = {}
    if flag_scat_length_neutron:
        dder["scat_length_neutron_real"] = (numpy.expand_dims(pr_2, axis=2)*\
            (pr_1*atom_multiplicity*atom_occupancy*debye_waller)).sum(axis=1)/pr_2.shape[-1]
    ...
    if flag_debye_waller:
        dder["debye_waller"] = (numpy.expand_dims(pr_2, axis=2)*\
            (pr_1*atom_multiplicity*atom_occupancy*scat_length_neutron)).sum(axis=1)/pr_2.shape[-1]
    if flag_atom_occupancy:
        dder["atom_occupancy"] = (numpy.expand_dims(pr_2, axis=2)*\
            (pr_1*atom_multiplicity*scat_length_neutron*debye_waller)).sum(axis=1)/pr_2.shape[-1]
    if flag_pr_1:
        dder["pr_1_real"] = (pr_2[:,:,na]*atom_multiplicity[na, na, :]*atom_occupancy[na, na, :]*scat_length_neutron*debye_waller)/pr_2.shape[-1]
        dder["pr_1_imag"] = 1j*(pr_2[:,:,na]*atom_multiplicity[na, na, :]*atom_occupancy[na, na, :]*scat_length_neutron*debye_waller)/pr_2.shape[-1]
    return res, dder
...
def calc_f_nucl(index_hkl,
    ...
    if (flag_use_precalculated_data and ("atom_multiplicity" in dict_in_out_keys)):
        ...
        atom_symm_elems = numpy.stack([
            (atom_fract_xyz[0]*10**5).astype(int),
            (atom_fract_xyz[1]*10**5).astype(int),
            (atom_fract_xyz[2]*10**5).astype(int),
            ones*10**5], axis=0)
    ...
    if (flag_use_precalculated_data and ("f_asym" in dict_in_out_keys) and
        ...
        f_asym, dder_f_asym = calc_f_asym_by_pr(
            atom_multiplicity, scat_length_neutron, debye_waller_factor, atom_occupancy,
            pr_1, pr_2, flag_scat_length_neutron=flag_scat_length_neutron,
    ...
    if (flag_use_precalculated_data and ("f_nucl" in dict_in_out_keys) and
        ...
        f_nucl, dder_f_nucl = calc_f_by_f_asym_pr(f_asym, pr_3, centrosymmetry, pr_4, flag_f_asym=flag_f_asym)
        if flag_dict:
...
def calc_sft_ccs(index_hkl,
    ...
    if (flag_use_precalculated_data and ("atom_para_multiplicity" in dict_in_out_keys)):
        ...
        atom_symm_elems = numpy.stack([
            (atom_para_fract_xyz[0]*10**5).astype(int),
            (atom_para_fract_xyz[1]*10**5).astype(int),
            (atom_para_fract_xyz[2]*10**5).astype(int),
            ones*10**5], axis=0)
        if "full_symm_elems" in dict_in_out_keys:
            full_symm_elems = dict_in_out["full_symm_elems"]
        else:
    ...
    if (flag_use_precalculated_data and ("sft_ccs_asym" in dict_in_out_keys) and
        ...
        sft_ccs_asym, dder_sft_ccs_asym = calc_sft_ccs_asym_by_pr(
            mag_atom_multiplicity, atom_para_form_factor, debye_waller_factor, atom_para_occupancy, atom_para_susceptibility, atom_para_sc_chi,
            pr_1, pr_2, pr_5, flag_atom_para_form_factor=flag_atom_para_form_factor,
    ...
    if (flag_use_precalculated_data and ("sft_ccs" in dict_in_out_keys) and
        ...
        pr_3_exp = numpy.expand_dims(pr_3, axis=0)
        pr_4_exp = numpy.expand_dims(pr_4, axis=0)
        sft_ccs, dder_sft_ccs = calc_f_by_f_asym_pr(sft_ccs_asym, pr_3_exp, centrosymmetry, pr_4_exp, flag_f_asym=flag_sft_ccs_asym)
        if flag_dict:
    ...
    if flag_atom_para_susceptibility:
        dder["atom_para_susceptibility"] = (
            dder_sft_ccs["f_asym_real"][:, na, :, na]*dder_sft_ccs_asym["atom_para_susceptibility"]+
            dder_sft_ccs["f_asym_imag"][:, na, :, na]*dder_sft_ccs_asym["atom_para_susceptibility"])
    
    return sft_ccs, dder


def calc_index_hkl_multiplicity_in_range(sthovl_min, sthovl_max, unit_cell_parameters, reduced_symm_elems, translation_elems):
    ...
    index_hkl_equivalent = calc_equivalent_reflections(index_hkl_full, reduced_symm_elems)
...
def calc_f_m_perp_ordered(index_hkl,
    ...
    if (flag_use_precalculated_data and ("atom_ordered_multiplicity" in dict_in_out_keys)):
        ...
        atom_symm_elems = numpy.stack([
            (atom_ordered_fract_xyz[0]*10**5).astype(int),
            (atom_ordered_fract_xyz[1]*10**5).astype(int),
            (atom_ordered_fract_xyz[2]*10**5).astype(int),
            ones*10**5], axis=0)
