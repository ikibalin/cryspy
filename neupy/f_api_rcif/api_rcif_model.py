...
def conv_rcif_to_model(rcif):
    ...
    model = Model(name=name_p_glob, file_dir=f_dir)
...
def conv_data_to_experiment_single(data, f_dir, l_crystal):
    ...
    observed_data = ObservedDataSingle(file_dir=f_dir)
    experiment = ExperimentSingle(setup=setup, 
...
def conv_data_to_experiment_single_domain(data, f_dir, l_crystal):
    beam_polarization = BeamPolarization()
    setup = SetupSingle(beam_polarization=beam_polarization)
    observed_data = ObservedDataSingleDomain(file_dir=f_dir)
    experiment = ExperimentSingleDomain(setup=setup, 
                                    observed_data=observed_data, file_dir=f_dir)
...
def conv_data_to_experiment_powder_1d(data, f_dir, l_crystal):
    beam_polarization = BeamPolarization()
    resolution = ResolutionPowder1D()
    factor_lorentz = FactorLorentzPowder1D()
    asymmetry = AsymmetryPowder1D()
    background = BackgroundPowder1D(file_dir=f_dir)
    setup = SetupPowder1D(resolution=resolution, 
    ...
    experiment = ExperimentPowder1D(setup=setup, 
                                    observed_data=observed_data, file_dir=f_dir)
...
def conv_data_to_experiment_powder_2d(data, f_dir, l_crystal):
    beam_polarization = BeamPolarization()
    resolution = ResolutionPowder2D()
    factor_lorentz = FactorLorentzPowder2D()
    asymmetry = AsymmetryPowder2D()
    background = BackgroundPowder2D(file_dir=f_dir)
    setup = SetupPowder2D(resolution=resolution, 
            factor_lorentz=factor_lorentz, asymmetry=asymmetry, 
            beam_polarization=beam_polarization, background=background)
        
    observed_data = ObservedDataPowder2D(file_dir=f_dir)
    
    experiment = ExperimentPowder2D(setup=setup, 
                                    observed_data=observed_data, file_dir=f_dir)
    ...
    for loop in data["loops"]:
        ...
        flag_exclude = "_2dpd_exclude_tth_min" in l_key
        if flag_scale:
            ...
            for i_crystal in range(n_crystal):
                ...
                for crystal in l_crystal:
                    if  dd["_2dpd_phase_name"] == crystal.get_val("name"):
                        ...
                        experiment.add_calculated_data(calculated_data)   
        if flag_exclude:
            l_excl_tth_min = [conv_str_to_text_float_logic(hh, "tth_min") for hh in loop["_2dpd_exclude_tth_min"]]
            l_excl_tth_max = [conv_str_to_text_float_logic(hh, "tth_max") for hh in loop["_2dpd_exclude_tth_max"]]
            l_excl_phi_min = [conv_str_to_text_float_logic(hh, "phi_min") for hh in loop["_2dpd_exclude_phi_min"]]
            l_excl_phi_max = [conv_str_to_text_float_logic(hh, "phi_max") for hh in loop["_2dpd_exclude_phi_max"]]
            experiment.set_val(excl_tth_min=l_excl_tth_min, excl_tth_max=l_excl_tth_max,
                               excl_phi_min=l_excl_phi_min, excl_phi_max=l_excl_phi_max)
    return experiment
...
def conv_data_to_experiment_powder_texture_2d(data, f_dir, l_crystal):
    ...
    experiment = ExperimentPowderTexture2D(setup=setup, 
                                    observed_data=observed_data, file_dir=f_dir)
    ...
    for loop in data["loops"]:
        ...
        flag_exclude = "_2dpdt_exclude_tth_min" in l_key
        if flag_scale:
            ...
            for i_crystal in range(n_crystal):
                ...
                for crystal in l_crystal:
                    if  dd["_2dpdt_phase_name"] == crystal.get_val("name"):
                        ...
                        experiment.add_calculated_data(calculated_data)   
        if flag_exclude:
            l_excl_tth_min = [conv_str_to_text_float_logic(hh, "tth_min") for hh in loop["_2dpdt_exclude_tth_min"]]
            l_excl_tth_max = [conv_str_to_text_float_logic(hh, "tth_max") for hh in loop["_2dpdt_exclude_tth_max"]]
            l_excl_phi_min = [conv_str_to_text_float_logic(hh, "phi_min") for hh in loop["_2dpdt_exclude_phi_min"]]
            l_excl_phi_max = [conv_str_to_text_float_logic(hh, "phi_max") for hh in loop["_2dpdt_exclude_phi_max"]]
            experiment.set_val(excl_tth_min=l_excl_tth_min, excl_tth_max=l_excl_tth_max,
                               excl_phi_min=l_excl_phi_min, excl_phi_max=l_excl_phi_max)
    return experiment
...
def conv_model_to_rcif(model):
    ...
    for obj in model._list_experiment:
        ...
        if isinstance(obj, ExperimentSingleDomain):
            l_relation = data_experiment_single_domain_relation()
            for relation in l_relation:
                temp_func(dd, relation, obj)
        ...
        if isinstance(obj, ExperimentPowderTexture2D):
            l_relation = data_experiment_powder_texture_2d_relation()
            for relation in l_relation:
                temp_func(dd, relation, obj)
        ...
        if isinstance(obj, ExperimentPowder2D):
            ...
            l_excl_tth_min = ["{:}".format(hh) for hh in obj._p_excl_tth_min]
            l_excl_tth_max = ["{:}".format(hh) for hh in obj._p_excl_tth_max]
            l_excl_phi_min = ["{:}".format(hh) for hh in obj._p_excl_phi_min]
            l_excl_phi_max = ["{:}".format(hh) for hh in obj._p_excl_phi_max]
            if len(l_excl_tth_min) != 0:
                d_eph_2 = {"_2dpd_exclude_tth_min":l_excl_tth_min, "_2dpd_exclude_tth_max":l_excl_tth_max,
                           "_2dpd_exclude_phi_min":l_excl_phi_min, "_2dpd_exclude_phi_max":l_excl_phi_max}
                lloop_d.append(d_eph_2)            
        ...
        if isinstance(obj, ExperimentPowderTexture2D):
            ...
            l_excl_phi_max = ["{:}".format(hh) for hh in obj._p_excl_phi_max]
            if len(l_excl_tth_min) != 0:
                d_eph_2 = {"_2dpdt_exclude_tth_min":l_excl_tth_min, "_2dpdt_exclude_tth_max":l_excl_tth_max,
                           "_2dpdt_exclude_phi_min":l_excl_phi_min, "_2dpdt_exclude_phi_max":l_excl_phi_max}
                lloop_d.append(d_eph_2)
...
def data_exclude_powder_texture_2d_relation():
    l_relation = [("_2dpdt_exclude_tth_min", "excl_tth_min", "val"),
        ("_2dpdt_exclude_tth_max", "excl_tth_max", "val"),
        ("_2dpdt_exclude_phi_min", "excl_phi_min", "val"),
        ("_2dpdt_exclude_phi_max", "excl_phi_max", "val")]
    return l_relation
